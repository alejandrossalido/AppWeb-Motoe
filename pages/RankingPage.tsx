...
const RankingPage: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      <Header title="Tabla de Clasificación" subtitle="Ranking de créditos y rendimiento" />
            ...
            <div key={user.id} className={`flex items-center gap-6 p-5 rounded-3xl border transition-all ${user.id === currentUser?.id ? 'bg-primary/10 border-primary/30 shadow-glow' : 'bg-surface-dark border-white/5 hover:border-white/10'}`}>
              ...
              <div className="flex flex-col items-end shrink-0">
                <div className="flex items-center gap-2 text-primary">
                  <span className="material-symbols-outlined text-[20px] font-black">stars</span>
                  <span className="text-2xl font-black tracking-tighter">{user.totalCredits}</span>
                </div>
                <p className="text-[9px] text-gray-600 font-bold uppercase tracking-widest">{user.totalHours.toFixed(1)}h Totales</p>
              </div>
            </div>
          ))}
        </div>
  ...
  <div className="flex flex-col items-center group">
    ...
    <div className={`w-32 ${height} ${bgColor} border-t-4 ${color} rounded-t-3xl flex flex-col items-center justify-end p-4 transition-all group-hover:bg-white/5`}>
      ...
      <div className="flex items-center gap-1 text-white">
        ...
        <span className="text-lg font-black tracking-tighter">{user.totalCredits}</span>
      </div>
    </div>
  </div>
);
